...
#for(i in 1:7)
	{
        ...
        #  								cat("Value of J in small-else block: ", j, "\n")
           								if(as.character(D1143_17[j,1])==as.character(D1143_17[j-1,1]) && as.numeric(D1143_17[j,2])==as.numeric(D1143_17[j-1,2]+1))
           									{
           										#l<- l+1    #this line is for sliding the window 1 Nycleotide position
           										l<- l+10    #this line is for sliding the window  10 Nycleotide position
            									break()
            								}
            							else
            								{
...
D1143_17_SW$Mean_depth<- D1143_17_SW$Mean_depth/75

Sys.time()
#write.table(D1143_17_SW, file = "~/Dropbox/sliding_window/D1143_17_SW75_10skip", sep = "\t", row.names = FALSE)
